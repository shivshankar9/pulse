...
const TeamTab = () => {
    ...
    const invite = async (event) => {
        ...
        if (!email.trim()) return toast.error("Enter a team member email");
        setSending(true);
        try {
            const { data } = await api.post("/invitations", { email: email.trim(), role });
            setInvites((current) => [data, ...current]);
            setEmail("");
            toast.success(data.email_sent ? "Invitation sent" : "Invitation created — copy the invite link from the pending list");
        } catch (error) {
            toast.error(error.response?.data?.detail || "Unable to send invitation");
        } finally {
            setSending(false);
        }
    };
    ...
    const removeInvite = async (id) => {
        try {
            ...
            toast.error(error.response?.data?.detail || "Unable to cancel invitation");
        }
    };
            ...
            <div className="bg-white border-2 border-gray-200 rounded-xl p-6 shadow-md">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                        ...
                        <p className="text-gray-600 mt-1">Invite teammates, assign access, and manage pending invitations.</p>
                    </div>
                    <span className="rounded-full bg-green-100 px-3 py-1 text-sm font-semibold text-green-800">{members.length} members</span>
                </div>
                <form onSubmit={invite} className="mt-6 grid gap-3 md:grid-cols-[1fr_180px_auto]">
                    <label className="sr-only" htmlFor="team-invite-email">Teammate email</label>
                    <input id="team-invite-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="teammate@company.com" className="rounded-lg border border-gray-300 px-4 py-3 text-gray-900 outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-200" />
                    <label className="sr-only" htmlFor="team-invite-role">Role</label>
                    <select id="team-invite-role" value={role} onChange={(e) => setRole(e.target.value)} className="rounded-lg border border-gray-300 px-4 py-3 text-gray-900 outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-200">
                        <option value="agent">Agent</option><option value="manager">Manager</option><option value="admin">Admin</option>
                    </select>
                    <button type="submit" disabled={sending} className="inline-flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-5 py-3 font-semibold text-white hover:bg-blue-700 disabled:opacity-60"><Plus className="h-4 w-4" />{sending ? "Sending..." : "Invite teammate"}</button>
                </form>
            </div>
