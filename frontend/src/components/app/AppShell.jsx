...
const AppShell = () => {
    ...
    return <div className="min-h-screen bg-bg selection:bg-brand selection:text-white"><aside className="fixed inset-y-0 left-0 z-40 hidden w-72 flex-col border-r border-border bg-surface md:flex">{nav}</aside>{mobileOpen && <><button type="button" aria-label="Close navigation overlay" onClick={() => setMobileOpen(false)} className="fixed inset-0 z-30 bg-ink/30 md:hidden" /><aside className="fixed inset-y-0 left-0 z-40 flex w-[min(88vw,22rem)] flex-col border-r border-border bg-surface shadow-2xl md:hidden">{nav}</aside></>}<main className="min-w-0 md:pl-72"><header className="sticky top-0 z-20 flex items-center justify-between border-b border-border bg-surface/95 px-4 py-3 backdrop-blur md:hidden"><button type="button" onClick={() => setMobileOpen(true)} aria-label="Open navigation" className="grid size-10 place-items-center rounded-lg border border-border text-ink hover:bg-bg"><Menu /></button><span className="font-heading font-black tracking-tight">BillByteKOT</span><button type="button" onClick={() => navigate("/app/contacts?new=1")} aria-label="Add customer" className="grid size-10 place-items-center rounded-lg border border-border text-ink hover:bg-bg"><Plus /></button></header><Outlet /></main></div>;
};
